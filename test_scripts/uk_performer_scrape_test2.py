...
def get_actor_info(actor_meta):
    data_file = open('data/uk_productions/uk-performers-with-bday-better.tsv', 'a')
    ...
    if len(actor_meta) > 1:
        ...
        if not person_not_found:
            ...
            birth_date = re.search(r'\(born\s(\d+\s\w+\s\d\d\d\d|\w+\s\d+\,\s\d\d\d\d|\d\d\d\d)', soup.find('p').get_text())
            if birth_date:
                #print(actor_name, birth_date.group(0))
                actor_info = birth_date.group(1) + '\t' + actor_info
            else:
                birth_date = re.search(r'(\d+\s\w+\s\d\d\d\d|\w+\s\d+\,\s\d\d\d\d)', soup.find('p').get_text())
                if birth_date:
